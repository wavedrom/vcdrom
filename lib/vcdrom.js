...
const getHandler = (content, inst) => async readers => {
  ...
  vcdPipeDeso({}, inst, deso => {
    ...
    container.elo.container.addEventListener('keydown', 
      genKeyHandler.genKeyHandler(content, container.pstate, render, cm, keyBindo,
        [websocket.keyPlugin]));
    container.elo.container.addEventListener('wheel', 
      genOnWheel(content, container.pstate, render, cm, keyBindo,
        [websocket.keyPlugin]));

    websocket.setup({ 
      render: render,
      pstate: container.pstate,
      xOffsetUpdate: xOffsetUpdate,
      getX: getX,
      getT: getT,
      cm: cm,
      keyBindo: keyBindo,
    ...
    mouse.setCursorMode(mouse.CursorMode.Click);
    mouse.plugins.push(
      websocket.mousePlugin
    );
  });
