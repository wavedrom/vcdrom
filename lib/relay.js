...
function createRelay(http_server, socket_port) {
  console.log("create relay");
  var wss = new WebSocket.Server({ server: http_server });
  ...
  var socket_server = net.createServer((server) => {
    socketServer = server;
    console.log(
  ...
  eventDebug(socket_server, "socket");

  // Open server on port 9898
  socket_server.listen(socket_port, () => {
    console.log("opened socket server on", socket_server.address().port);
  });
}
