...
const renderBookMark = (parent_div, pstate) => {
  ...
  for (let i = 0; i < pstate.bookmarks.length; i++) {
    ...
    console.log(pstate.bookmarks[i]);
    const label =
      pstate.bookmarks[i].note == ''
        ? pstate.bookmarks[i].time.toString()
        : pstate.bookmarks[i].time.toString() + ': ' + pstate.bookmarks[i].note;

    const lWidth = label.length * markFontWidth + 5;
    console.log(lWidth, label.length, markFontWidth);
