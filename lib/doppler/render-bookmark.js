...
function getLabel(bookmark, pstate) {
  console.log(bookmark);

  if (bookmark.note === '') {
    return formatTime(bookmark.time, pstate.timescale);
  }
...
const renderBookMark = (parent_div, pstate) => {
  ...
  for (let i = 0; i < pstate.bookmarks.length; i++) {
    ...
    const label = getLabel(pstate.bookmarks[i], pstate);
    ...
    const body = [
      ...
      [
        'rect',
        {
          ...
          x: 0,
          y:
            height - yMargin - markFontHeight * 1.25 - pstate.bottomLabelOffset,
          width: lWidth,
      ...
      [
        'text',
        {
          ...
          y:
            height -
            (yMargin + markFontHeight * 0.25) -
            pstate.bottomLabelOffset,
        },
        label,
      ],
    ];
