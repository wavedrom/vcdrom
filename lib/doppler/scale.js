...
const up = delta => (pstate, event, cm) => {
...
const down = delta => (pstate, event, cm) => {
  const info = cm.getScrollInfo();
  cm.scrollTo(null, info.top + info.clientHeight * delta);
...
function getMult(event) {
  if (event === undefined) {
    // No scaling for mouse events....yet
    return 1.0;
  }

  return (event.shiftKey ? 10 : 1) * (event.ctrlKey ? 10 : 1);
}
...
module.exports = {
  ...
  ArrowLeft: (pstate, event) => {
    let { xOffset, sidebarOffset } = pstate;
    if (xOffset === sidebarOffset) {
      return false;
    }
    xOffset += 0.1 * getMult(event);
    if (xOffset > sidebarOffset) {
