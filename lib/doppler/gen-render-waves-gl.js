...
const renderMarks = require("./render-mark.js");
const renderBookMark = require("./render-bookmark.js");
...
const vertexShaderScalar = shaderer(
  "VERTEX_SHADER",
  `#version 300 es
in uvec3 pos;
out vec4 v_color;
...
const initProgram = (gl) => {
  ...
  return "colors tilts scale offset tilt".split(/\s+/).reduce(
    (res, name) =>
      Object.assign(res, {
      ...
      for (let i = 0; i < ilen; i++) {
        ...
        switch (val) {
          ...
          case 5: // z Z
            ...
            break;
          case 6:
          case 7: // u U uninitialized
            vertices.push(...bar[2](f[1], t[1], tim, tt, 6, 6));
            break;
          case 8:
          case 9: // w W weak unknown
  ...
  return (desc) => {
    ...
    return (pstate) => {
      ...
      return () => {
        ...
        if (!doRender) {
          // rendering is disabled, do nothing
          return;
        }
        ...
        renderMarks(marka, "a", pstate);
        renderMarks(markb, "b", pstate);
        renderMarks(markc, "c", pstate);
        renderMarks(markd, "d", pstate);
        renderBookMark(bookmark, pstate);
