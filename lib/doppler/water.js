  ...
  for (let k = 0; k < 2000; k++) {
    ...
    signals.map((sig) => {
      ...
      if (t === cursor) {
        ...
        if (le) {
          if (pco[pc] === undefined) {
            if (Object.keys(pco).length > 5 * len) {
              return;
            }
            pco[pc] = { op: le.op, asm: le.asm, bricks: [] };
          }
...
const water = (lane, desc, pstate) => {
  ...
  for (let j = 0; j < lane.len - 2; j++) {
    ...
    if (j & 1) {
      mLane.push([
        'rect',
        { width: width, height: yStep2 - 1, class: 'pc-odd' },
      ]);
    ...
    mLane.push([
      'text',
      { class: 'pc', 'xml:space': 'preserve', y: Math.round(yStep2 * 0.7) },
      [
        'tspan',
        { class: 'pc-addr' },
        parseInt(pc, 10).toString(16).padStart(12, ' '),
      ],
      ...
      mLane.push([
        ...
        ...(brickWidth > 20
          ? [
              [
                'text',
                { class: e.name, width: brickWidth, x: brickWidth / 2, y: 16 },
                e.label,
              ],
            ]
          : []),
