...
const renderCursor = (cfg, pstate) => {
  const {cursorXmargin: xmargin, cursorFontHeight: fontHeight, cursorFontWidth: fontWidth, height, timescale} = pstate;
  ...
  const body = [
    ...
    [
      "rect",
      {
        class: "wd-cursor-time",
        x: xmargin - lWidth / 2,
        y: height - yMargin - fontHeight * 1.25,
        width: lWidth,
        height: fontHeight * 1.25,
      },
    ],
    [
      "text",
      {
        class: "wd-cursor-time",
        x: xmargin,
        y: height - (yMargin + fontHeight * 0.25),
      },
      label,
    ],
  ];
  if (pstate.markmode) {
    pstate.ws.sendmsg('marked ' + xx);
  }
