...
const renderMark = (div, key, pstate) => {
  ...
  // console.log("render0marks", tStartExact, tFinishExact, tMark);
  if (tMark == -1) {
    // No mark set yet.
  ...
  const body = [
    ...
    [
      'rect',
      {
        class: 'wd-' + markClass + '-time',
        x: markXMargin - lWidth / 2,
        y: height - yMargin - markFontHeight * 1.25 - pstate.bottomLabelOffset,
        width: lWidth,
        height: markFontHeight * 1.25,
      },
    ],
    [
      'text',
      {
        ...
        x: markXMargin,
        y:
          height - (yMargin + markFontHeight * 0.25) - pstate.bottomLabelOffset,
      },
      label,
    ],
  ];
